...
export class OpenAIService implements ILLMService {
     ...
     * Emits 'llmservice:chunk' events for each text chunk as it's generated.
     *
    ...
    async completeTaskStreaming(userInput: string, imageData?: ImageData): Promise<void> {
        ...
        let iterationCount = 0;
        // Initialize fullResponse for the first iteration
        let fullResponse = '';
        ...
        try {
            while (iterationCount < this.maxIterations) {
                ...
                // Reset fullResponse at the start of each iteration
                // This prevents combining responses across tool calls
                fullResponse = '';
                ...
                for await (const chunk of stream) {
                    ...
                    if (content) {
                        ...
                        this.eventEmitter.emit('llmservice:chunk', content);
                    }
                ...
                if (toolCalls.length === 0) {
                    this.eventEmitter.emit('llmservice:response', assistantMessage);
                    return;
                }
            ...
            // Handle API errors
            const errorMessage = error instanceof Error ? error.message : String(error);
            logger.error(`Error in OpenAI streaming API call: ${errorMessage}`, { error });
            logger.warn(
                `If this error is due to token overflow, consider configuring 'maxTokens' in your LLMConfig.`
            );
            this.eventEmitter.emit(
