...
This guide explains how to add support for a new Large Language Model (LLM) provider to Ace. The LLM service architecture is designed to be extensible, allowing you to integrate with different AI providers while maintaining consistent behavior.

## LLM Architecture Overview

Ace uses an abstraction layer to interact with different LLM providers:
...
7.  **Test:** Configure and run Ace to test your new provider integration.
...
*   **Role:** Translates Ace's internal message history (`InternalMessage[]`) into the specific array format required by your chosen LLM provider's API.
...
export class YourProviderService implements ILLMService {
    ...
    // Translates Ace's internal ToolSet into the provider's specific format
...
1.  **Configure `ace.yml`:** Update your configuration file to use your new provider:
   ```yaml
...
3.  **Run Ace:** Start the application (`npm start` or similar).
4.  **Test Thoroughly:**
    *   Send simple prompts.
...
By following these steps, you should be able to successfully integrate a new LLM provider into Ace's extensible architecture. Remember to adapt the provider-specific logic (`formatToolsForProvider`, `parseProviderResponse`, API calls) based on the chosen provider's SDK and API documentation.