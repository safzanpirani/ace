...
export class MCPClient implements IMCPClient {
    ...
    async connectViaStdio(
        ...
        // Add environment variables to suppress MCP server verbose logging
        const expandedEnv = {
            ...process.env,
            // Suppress Python logging from MCP servers
            PYTHONIOENCODING: 'utf-8',
            LOG_LEVEL: 'ERROR',
            MCP_LOG_LEVEL: 'ERROR',
            // Suppress other common verbose logging
            RUST_LOG: 'error',
            NODE_ENV: 'production',
            ...(env || {}),
        };
    ...
    private async connectViaHttp(
        ...
        this.client = new Client(
            { name: 'Ace-http-mcp-client', version: '1.0.0' },
            { capabilities: { tools: {} } }
        );
        try {
