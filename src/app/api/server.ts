...
export async function initializeApi(agent: AceAgent, agentCardOverride?: Partial<AgentCard>) {
    const app = express();
    ...
    const baseApiUrl = process.env.ACE_BASE_URL || `http://localhost:${process.env.PORT || 3000}`;
    const agentCardData = createAgentCard(
        {
            defaultName: overrides.name ?? 'ace',
